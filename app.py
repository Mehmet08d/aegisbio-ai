...
with tab_analiz:
    ...
    if run_button:
        ...
        # Tüm 13 Gen Seçildiyse
        if selected_code == "ALL_13":
            all_genes = ["ND1", "ND2", "ND3", "ND4", "ND4L", "ND5", "ND6", "CYTB", "COX1", "COX2", "COX3", "ATP6", "ATP8"]
            all_results = []
            all_farklar = []

            with st.spinner("Tüm 13 mitokondriyal gen NCBI veritabanından çekilip taranıyor..."):
                for g in all_genes:
                    res = gen_analiz_et(g, numbat_acc, thylacine_acc)
                    if res:
            ...
            # Tekli Gen Seçildiyse
            with st.spinner(f"{selected_code} geni çekiliyor..."):
                res = gen_analiz_et(selected_code, numbat_acc, thylacine_acc)
                if res:
...
with tab_ai_bot:
    ...
    if user_prompt := st.chat_input("Biyoinformatik veya genetik mühendisliği ile ilgili sorunuzu yazın..."):
        ...
        if client:
            ...
            # API Key Olmadığında Akıllı Simülasyon Yanıtı
